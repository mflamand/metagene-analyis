...
install.packages("tidyverse") # ggplot comes with tidyverse, no need to reinstall.
...
Here, I filter transcript by length to only keep one
```{r}
# add the length of all section to get the full length
m6a.dist <- m6a.dist %>% mutate(., len = utr5_size + cds_size + utr3_size)

# group all sites by gene_name, than arrange them from longuest to shortest and only keep the longest gene_name, for unique coordinates (sites) 
...
```{r}
qplot(m6a.dist$rel_location, geom="histogram", bins=75) +
  geom_vline(xintercept = 1:2, col = "grey") +
  theme_bw()
```

qplots works well, but has to guess what exactly what we want. To instead get more granular control, we can use the full fledge ggplot2 to plot what we want
...
ggsave("Metagene.lander.m6A.sites.jpeg", height = 2.5, width = 4.5, dpi = 600)
```
...
m6a.shuffle <- m6a.dist %>% mutate(scaled_location = sample(seq(from=min(m6a.dist$scaled_location),    #same range of values
                                                                to = max(m6a.dist$scaled_location),
                                                                by = 0.001),
                                                            length(m6a.dist$scaled_location),          # maximum number of values
                                                            replace=T)) %>% 
...
combined<- bind_rows(m6a.dist, m6a.shuffle) #combine the rows of both datasets (as a unix Cat would do except that here you need the same columns to be present in both files)
```


###The data can now be separated by colors based on the tag in the newly created column ("col")
It is usually much easier to plot something with ggplot when the data is tidy. You will want all the observations to be in the same column for all your data.
```{r}
